...
Original file is located at
    https://colab.research.google.com/drive/1QTCSjGuLI9mAbqRcnUwhFWVMlO8K0Ght
"""
...
def main():
    ...
    coef_var_casa_color = colored(coef_var_casa_formatted, 'green') if coef_var_casa < 0.70 else colored(coef_var_casa_formatted, 'red')
    coef_var_fora_color = colored(coef_var_fora_formatted, 'green') if coef_var_fora < 0.70 else colored(coef_var_fora_formatted, 'red')

    # Cria a tabela com os resultados
    tabela = [
        ['Time', 'Média de Gols', 'Desvio Padrão', 'Coeficiente de Variação'],
        ['Casa', f'{media_casa:.2f}', f'{desvio_padrao_casa:.2f}', coef_var_casa_color],
        ['Fora', f'{media_fora:.2f}', f'{desvio_padrao_fora:.2f}', coef_var_fora_color]
    ]
